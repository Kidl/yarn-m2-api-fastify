...
const redis = process.env.REDIS_URL
  ? new Redis(process.env.REDIS_URL, { keyPrefix: process.env.REDIS_KEY_PREFIX })
  : new Redis({ host: process.env.REDIS_HOST, keyPrefix: process.env.REDIS_KEY_PREFIX });
...
async function set(_arguments, data, EX = process.env.REDIS_EX || 86400 ) {
  if (process.env.CACHE === 'true') {
    const key = getKey(_arguments.callee, _arguments);
