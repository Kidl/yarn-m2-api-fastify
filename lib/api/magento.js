...
const CONFIGURABLE_PRODUCTS_ENDPOINT_URL = process.env.CONFIGURABLE_PRODUCTS_ENDPOINT_URL;
...
async function getAttributes() {
  const uri = PRODUCTS_ENDPOINT_URL + '/attributes?searchCriteria[filter_groups][0][filters][0][field]=attribute_code' +
    '&searchCriteria[filter_groups][0][filters][0][value]=brand,gauge,gauge_stockinette,fiber_content,fabric_care,color_tint,certificates' +
...
async function getConfigurableProductBySku(sku) {
  const uri = `${CONFIGURABLE_PRODUCTS_ENDPOINT_URL}/${encodeURIComponent(sku)}/children`;
