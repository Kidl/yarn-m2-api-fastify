...
function getId(variable) {
  ...
  const attributeSetId = Object.entries(ATTRIBUTE_SETS).filter(entry => entry[0].toString() === variable.toString())[0][1];
...
function getType(id) {
    ...
    const ATTRIBUTE_SETS = str2obj(process.env.ATTRIBUTE_SETS);
    const attributeSetType = Object.entries(ATTRIBUTE_SETS).filter(entry => entry[1].toString() === id.toString())[0][0];
